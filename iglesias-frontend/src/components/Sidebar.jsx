export default function Sidebar({
  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 border-b border-neutral-100 dark:border-neutral-800 flex justify-between items-center shrink-0">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-1.5 rounded-lg text-neutral-400 dark:text-neutral-500 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all active:scale-90"
          >
            {darkMode ? (
          ...
          <button
            onClick={onClose}
            title="Cerrar"
            className="md:hidden p-1.5 rounded-lg text-neutral-400 dark:text-neutral-500 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all active:scale-90"
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* Zone switcher */}
      <div className="px-3 py-3 border-b border-neutral-100 dark:border-neutral-800 flex flex-col gap-1 shrink-0">
        {zonas.map((z) => (
          <button
            key={z.id}
      ...
      {/* Church list */}
      <div className="flex-1 overflow-y-auto overscroll-contain px-3 py-3 w-full">
        {loading ? (
          <div className="flex flex-col gap-y-3 pt-1">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="w-full h-24 rounded-xl bg-neutral-100 dark:bg-neutral-800 animate-pulse"
              />
            ))}
          </div>
        ) : iglesias.length === 0 ? (
          ...
          <div key={activeZone} className="flex flex-col gap-y-3 animate-fade-in">
            {iglesias.map((ig) => (
              <div
                key={ig.id}
      ...
      <button
        onClick={onAdd}
        className="mx-3 mb-3 py-2.5 rounded-xl border border-dashed border-neutral-200 dark:border-neutral-700 text-sm text-neutral-400 dark:text-neutral-500 hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-all active:scale-[0.98] shrink-0"
      >
        + Agregar iglesia
      </button>
    </aside>
